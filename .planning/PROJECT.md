# GPUVault — The Two-Sided Compute Marketplace with Intelligent Routing

## What This Is

GPUVault is a two-sided compute marketplace powered by an AI routing agent. For buyers (developers, researchers, teams), it eliminates decision fatigue by scanning every connected provider, normalizing different pricing models, and recommending the best deal in seconds. For sellers (organizations with idle GPUs), it provides an easy on-ramp to monetize spare capacity without building their own marketplace.
...
- [ ] **Multi-Provider Price Aggregation** — Expand beyond Akash to io.net, AWS Spot, Render, Nosana, and GPUVault-listed providers
